...
def QNet(scrshot_size, action_size) :
    ...
    x = Conv2D(32, (6, 6), padding = "valid", activation = LeakyReLU(0.1))(input_scrshots)
    x = MaxPooling2D((4, 4), padding = "same")(x)
    x = Conv2D(64, (4, 4), padding = "valid", activation = LeakyReLU(0.1))(x)
    x = MaxPooling2D((2, 2), padding = "same")(x)
    x = Conv2D(128, (3, 3), padding = "valid", activation = LeakyReLU(0.1))(x)
    x = Flatten()(x)
