...
class ActorCritic:
    def __init__ (self, load_weight_name, scrshot_size):    
        ...
        self.valueNet.compile(loss = "mse", optimizer = self.q_optimizer)
    ...
    def initModels(self):
        ...
        self.valueNet = Model(input_scrshots, pred_q)
        self.valueNet.summary()
    ...
    def learn(self, trn_s, trn_a, trn_r):
        ...
        for j in range(cfg.train_size):
            pred_new_q = self.valueNet.predict(np.expand_dims(trn_s[j+1], axis=0))
            new_q[j] = trn_r[j] + cfg.gamma * pred_new_q
        trn_s = trn_s[:cfg.train_size]
        trn_a = trn_a[:cfg.train_size]
        loss = self.valueNet.train_on_batch(trn_s, new_q)
        return loss
